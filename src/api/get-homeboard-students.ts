...
import { addIfNotExist, LocalStorageKey, get, add } from "shared/helpers/local-storage"
...
function getExistingInfo(limit = 10, offset = 0, sortByKey = "first_name", sortOrder: "asc" | "dsc" = "asc", filter: string = "") {
  const data: [] = get(LocalStorageKey.students) ?? []
  const filteredData = filter ? data.filter(({ roll }) => roll === filter) : data
...
const getCount = (students: any, rollType: string) => students.filter(({ roll }: { roll: string }) => roll === rollType).length
...
export async function getHomeboardStudents(obj?: P) {
  ...
  try {
    ...
    return {
      ...
      total: TOTAL_COUNT,
      students: students
        ? getExistingInfo(obj?.limit, obj?.offset, obj?.sortByKey, obj?.sortOrder, obj?.filter)
        : addIfNotExist(LocalStorageKey.students, generateStudents(TOTAL_COUNT)),
      counts: {
...
export async function updateHomeboardStudent(newDetail: Person, studentId: number, obj: any) {
  try {
    await httpMock({ randomFailure: true })
...
export async function saveRollsInStorage() {
  try {
    await httpMock({ randomFailure: true })
    const students: [any] | [] = get(LocalStorageKey.students) ?? []
    const onlyRolledStudents = students?.filter(({ roll }) => roll !== "un-rolled")

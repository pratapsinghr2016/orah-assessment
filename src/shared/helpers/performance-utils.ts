...
export const sortBy = (data: any[any], sortByKey: string, sortOrder: "asc" | "dsc") =>
...
export function searchName(arr: any[any], searchedValue: string) {
  const filteredStudentList = arr.filter(
    ({ first_name: firstName, last_name: lastName }: keyProps) => lc(firstName).includes(lc(searchedValue)) || lc(lastName).includes(lc(searchedValue))
  )
